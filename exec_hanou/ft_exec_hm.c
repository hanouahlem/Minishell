...
/*   Updated: 2024/11/26 14:59:20 by manbengh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_exec_ve(t_mini *data)
{
	...
	ft_printf("je suis dans exec_ve \n");
	...
	{
		// ft_printf("=========================================== %s\n", data->parser->cmd);
		return (free_tab(way), free_tab(data->parser->args), free_resources(data), exit(1));
	}
	ft_printf("data->parser->args[0] : %s\ncmd : %s\n", data->parser->args[0], data->parser->cmd);
	if (execve(data->parser->cmd, data->parser->args, data->exec->env_exec) < 0)
	{
...
void	init_exec(t_mini *data)
{
	...
	if (!data->exec->pid)
	{
		perror("Malloc pid failed !\n");
		free_resources(data);
		exit(EXIT_FAILURE);
	}
	data->exec->pipes = malloc(sizeof(int *) * data->parser->size_cmd - 1);
	if (!data->exec->pipes)
	{
		perror("Malloc pipes failed !\n");
		free_resources(data);
		exit(EXIT_FAILURE);
	}
